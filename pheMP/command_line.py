...
def generate_keypair(keysize, id, output):
    ...
    pub, priv = pheMP.generate_paillier_keypair(n_length=keysize)
    ...
    jwk_public = {
        ...
        'alg': "PAI-GN1",
        "key_ops": ["encrypt"],
        'n': pheMP.util.int_to_base64(pub.n),
    ...
    jwk_private = {
        'kty': "DAJ",
        'key_ops': ["decrypt"],
        'p': pheMP.util.int_to_base64(priv.p),
        'q': pheMP.util.int_to_base64(priv.q),
        'pub': jwk_public,
...
def decrypt(private, ciphertext, output):
    ...
    _p = pheMP.util.base64_to_int(privatekeydata['p'])
    _q = pheMP.util.base64_to_int(privatekeydata['q'])

    private_key = pheMP.PaillierPrivateKey(pub, _p, _q)
...
def load_public_key(public_key_data):
    ...
    n = pheMP.util.base64_to_int(public_key_data['n'])
    pub = pheMP.PaillierPublicKey(n)
    return pub
