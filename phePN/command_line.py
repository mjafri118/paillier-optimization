...
def generate_keypair(keysize, id, output):
    ...
    pub, priv = phePN.generate_paillier_keypair(n_length=keysize)
    ...
    jwk_public = {
        ...
        'alg': "PAI-GN1",
        "key_ops": ["encrypt"],
        'n': phePN.util.int_to_base64(pub.n),
    ...
    jwk_private = {
        'kty': "DAJ",
        'key_ops': ["decrypt"],
        'p': phePN.util.int_to_base64(priv.p),
        'q': phePN.util.int_to_base64(priv.q),
        'pub': jwk_public,
...
def decrypt(private, ciphertext, output):
    ...
    _p = phePN.util.base64_to_int(privatekeydata['p'])
    _q = phePN.util.base64_to_int(privatekeydata['q'])

    private_key = phePN.PaillierPrivateKey(pub, _p, _q)
...
def load_public_key(public_key_data):
    ...
    n = phePN.util.base64_to_int(public_key_data['n'])
    pub = phePN.PaillierPublicKey(n)
    return pub
